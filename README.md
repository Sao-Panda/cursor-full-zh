...
> **适配版本**：Cursor `3.8.24`（内置 VS Code `1.105.1`，验证于 2026-06-26）；`3.8.23` / `3.8.11` 亦验证通过。同系列 `3.8.x` 小版本通常可直接复用。  
...
- **操作系统**：macOS（需已安装 `Cursor.app`，默认路径 `/Applications/Cursor.app`）
- **Python**：3.8+（通过 shell 脚本调用）
...
| 变量 | 含义 |
| --- | --- |
| `CURSOR_INSTALL_DIR` | Cursor 安装路径（如 `/Applications/Cursor.app`） |
| `CURSOR_USER_DATA_DIR` | 用户数据目录（默认 `~/Library/Application Support/Cursor`） |

也可在 `Cursor_Localization_Tool.py` 顶部 **用户配置区域** 写死路径，例如：

```python
CURSOR_AN_ZHUANG_LU_JING = "/Applications/Cursor.app"
CURSOR_SHU_JU_LU_JING = os.path.expanduser("~/Library/Application Support/Cursor")
```
...
| `启动汉化_Mac.sh` / `取消汉化_Mac.sh` | macOS 快捷脚本 |
| `Cursor_Setting_Lookup.js` | 辅助查找设置页英文原文 |
| `localization/翻译文档.md` | **维护向翻译文档**（界面层级 L0–L5、词典映射、版本对照；适配 Cursor `3.8.11`，文档版本 `1.0.1`） |

## 维护翻译

修改对应文件后，运行 `python Cursor_Localization_Tool.py` 并重启 Cursor。界面层级与词典归属详见 [localization/翻译文档.md](localization/翻译文档.md)（适配 Cursor `3.8.11`，文档版本 `1.0.1`）。
...
| 现象 | 处理 |
| --- | --- |
| 运行脚本报错 / 汉化无输出 | 确认已安装 **Python 3.8+**，终端执行 `python3 --version` 验证；也可用 `python Cursor_Localization_Tool.py --check-python` 检查环境 |
...
| --- | --- | --- | --- |
| 2026-06-26 | `3.8.24` | `1.105.1` | 验证通过；补充钩子/MCP/自动化/身份验证等约 120 条词条 |
